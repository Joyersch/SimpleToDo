...
class Entry extends DatabaseObject
{
    ...
    public function GetByID($ID) {
        ...
        $result = $this-> connection -> query("SELECT Text, Done, LinkedToDo, LinkedEntry FROM Entry WHERE ID = $ID LIMIT 0,1");
    ...
    public function Add() {
        ...
        return $this -> connection -> query(@"INSERT INTO Entry (Text{$queryAdditionalValues}) VALUES ('{$this -> Text}'{$queryAdditionalSet})");
    }
    ...
    public function GetUnderlyingEntries(){
        ...
        $result = $this -> connection -> query(@"SELECT ID FROM Entry WHERE LinkedToDo = {$this -> LinkedToDo } AND LinkedEntry = {$this -> ID}");
