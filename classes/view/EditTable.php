...
class EditTable
{
    ...
    public function Print($authentication)
    {
        ...
        $input->value = "c";
        $input->onclick =
            "redirectToCreateEntry(
                document
                    .getElementById(\"new_entry\")
                    .value
                , {$this->todo->ID}
                , \"$authentication\"
                , reloadPageIfGood);";
    ...
    public function PrintRows($entry, $total_depth, $current_depth, $authentication)
    {
        ...
        $input->value = "e";
        $input->onclick = "
            redirectToUpdateEntry(
                document.getElementById(\"entry_{$entry->ID}\")
                    .value
                , {$entry->ID}
                , \"$authentication\"
                , reloadPageIfGood);";
        ...
        $input->value = "c";
        $input->onclick = "
            redirectToCreateSubEntry(
                document.getElementById(\"new_entry_{$entry->ID}\").value
                , {$this->todo->ID}
                , {$entry->ID}
                , \"$authentication\"
                , reloadPageIfGood);";
